...
import type { Props, VirtualItem } from './type';
 ...
 */
const useUpdateVirtualItems = <T = any>(props: Props<T>, virtualItemsRef: ShallowRef<VirtualItem<T>[]>) => (
 ...
 * @returns 是否是垂直方向
 */
const isVerticalDirGetter = <T>(props: Props<T>) => props.direction === 'vertical';
...
const useResetSideWinPos = <T>(slideWinPos: SlideWindowOnAxis, props: Props<T>) => () => {
  slideWinPos.resetAxis(props.listData.length, props.arerageSpace ?? 10);
};

// 组件相关属性
export const useState = <T = any>(props: Props<T>) => {
  // 虚拟滚动可视容器dom元素
